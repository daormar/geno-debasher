...
def take_pars():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "s:a:t:f:p:v", [
                                   "sampleinfofile=", "analysisinfofile=", "studyinfofile=", "format=", "asperacontent="])
...
def get_info_in_basic_format(sample_info_map, analysis_info_map, study_info_map, aspera_box_info):
    ...
    for sample_accession in study_info_map:
        ...
        formatted_info.append((sample_accession, ega_sample_id, fileaccession,
                              filename, aspera_filename, donor_id, phenotype, gender))
...
def group_formatted_info_by_donor(formatted_info, field_list):
    ...
    for elem in formatted_info:
        ...
        if(elem[donor_id_idx] in group_map):
            group_map[elem[donor_id_idx]].append(
...
def format_info(format, sample_info_map, analysis_info_map, study_info_map, aspera_box_info):
    if(format == 1):
        ...
        formatted_info = get_info_in_basic_format(
            sample_info_map, analysis_info_map, study_info_map, aspera_box_info)
...
def process_pars(flags, values):
    ...
    # Format information
    formatted_info = format_info(
        values["format"], sample_info_map, analysis_info_map, study_info_map, aspera_box_info)
