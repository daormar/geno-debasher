...
def take_pars():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "m:f:c:i", [
                                   "mapf=", "file=", "col=", "invert"])
...
def print_help():
    ...
    print(
        "-f <string>    File where mapping needs to be done (if not given,", file=sys.stderr)
...
def process_pars(flags, values):
    ...
    map_contigs(values["file"], contig_map, inv_contig_map,
                values["col"], flags["invert"])
