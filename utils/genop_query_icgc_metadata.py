...
def take_pars():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "d:a:t:f:v", [
                                   "donorinfofile=", "awsmanif=", "table=", "format="])
...
def print_help():
    ...
    print("                4: Same as 3 but mini-bam files are excluded",
          file=sys.stderr)
    ...
    for file_id in awsmanif_map:
        ...
        if(not exclude_mini_bam_files or ("mini" not in filename)):
            formatted_info.append(
                (file_id, object_id, filename, donor_id, phenotype_code, donor_sex))
...
def format_info(format, donor_info_map, awsmanif_map, table_map):
    if(format == 1):
        ...
        formatted_info = get_info_in_basic_format(
            donor_info_map, awsmanif_map, table_map, exclude_mini_bam_files)
...
def process_pars(flags, values):
    ...
    formatted_info = format_info(
        values["format"], donor_info_map, awsmanif_map, table_map)
